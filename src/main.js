...
const SAVE_KEY = "bloxcraft-save-v1";
...
ui.startBtn.addEventListener("click", () => {
  ...
  if (saved) {
    selectedBlockType = saved.selectedBlockType || selectedBlockType;
...
function tick() {
  ...
  if (gameState === "playing") {
    ...
    ui.setStatus(
      `BloxCraft • XYZ ${player.position.x.toFixed(1)} ${player.position.y.toFixed(1)} ${player.position.z.toFixed(1)} • ${selectedBlockType} • Chunks ${world.generatedChunks.size}`
    );
