...
scene.add(sun);
scene.add(new THREE.HemisphereLight(0xdcefff, 0x8fb07f, 0.52));
...
const water = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ color: 0x3d89d8, transparent: true, opacity: 0.58, roughness: 0.2, metalness: 0.08 })
);
...
for (let i = 0; i < 20; i += 1) {
  const cloud = new THREE.Mesh(
    new THREE.BoxGeometry(6 + Math.random() * 10, 1.5 + Math.random() * 1.8, 3 + Math.random() * 7),
    new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 1 })
  );
  cloud.position.set((Math.random() - 0.5) * WORLD_SIZE * 2, 25 + Math.random() * 12, (Math.random() - 0.5) * WORLD_SIZE * 2);
  scene.add(cloud);
}
