...
export class World {
  ...
  queueChunk(cx, cz) {
    ...
    if (this.generatedChunks.has(ck) || this.queuedChunks.has(ck)) return;
    this.chunkQueue.push({ cx, cz });
    this.queuedChunks.add(ck);
  }
    ...
    for (let ring = 0; ring <= radius; ring += 1) {
      for (let dx = -ring; dx <= ring; dx += 1) {
        for (let dz = -ring; dz <= ring; dz += 1) {
          ...
          this.queueChunk(centerChunkX + dx, centerChunkZ + dz);
        }
      }
    }
  }

  ensureChunksAround(worldX, worldZ, radius) {
  ...
  serializePlayerChanges() {
    const data = [];
    for (const mesh of this.meshList) {
      data.push({ x: mesh.userData.x, y: mesh.userData.y, z: mesh.userData.z, type: mesh.userData.type });
    }
    return data;
  }

  restoreFromSave(entries) {
    for (const entry of entries) {
      this.addBlock(entry.x, entry.y, entry.z, entry.type);
    }
  }
