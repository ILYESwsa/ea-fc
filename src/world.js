...
export class World {
  ...
  getMaterialFor(type) {
    if (type === "grass") {
      return [sideGrass, sideGrass, topGrass, dirtMat, sideGrass, sideGrass];
    }
  ...
  generate() {
    ...
    for (let x = -half; x < half; x += 1) {
      for (let z = -half; z < half; z += 1) {
        const ridge = Math.sin((x - z) * 0.04) * 0.8;
        ...
        if (Math.random() < 0.03 && h > SEA_LEVEL + 1) {
  ...
  getGroundHeight(x, z) {
    for (let y = MAX_HEIGHT + 20; y >= 0; y -= 1) {
