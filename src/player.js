...
export class PlayerController {
  ...
  update(dt) {
    ...
    const bob = desired.lengthSq() > 0 ? Math.sin(this.walkTime) * 0.02 : 0;
    this.viewModel.position.set(0, bob, 0);
    this.viewModel.rotation.x = bob * 2.2;
  }
  ...
  resolveHorizontalCollision() {
    ...
    for (const [sx, sz] of samples) {
      ...
      if (this.world.hasBlock(bx, feetY, bz) || this.world.hasBlock(bx, headY, bz)) {
        ...
        if (Math.abs(dx) > Math.abs(dz)) this.position.x = bx + Math.sign(dx) * (0.5 + PLAYER_RADIUS + 0.001);
        else this.position.z = bz + Math.sign(dz) * (0.5 + PLAYER_RADIUS + 0.001);
      }
    }
  }
