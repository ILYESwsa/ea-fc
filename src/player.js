...
export class PlayerController {
  ...
  createAvatar() {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.8, 1.2, 0.45), new THREE.MeshStandardMaterial({ color: 0x3f7ddb }));
    body.position.y = 1.2;
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.62, 0.62), new THREE.MeshStandardMaterial({ color: 0xf0caa8 }));
    head.position.y = 2.1;
    const leg1 = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.9, 0.28), new THREE.MeshStandardMaterial({ color: 0x23457b }));
    const leg2 = leg1.clone();
    leg1.position.set(-0.16, 0.45, 0);
    leg2.position.set(0.16, 0.45, 0);
    const arm1 = new THREE.Mesh(new THREE.BoxGeometry(0.24, 1, 0.24), new THREE.MeshStandardMaterial({ color: 0xf0caa8 }));
    const arm2 = arm1.clone();
    arm1.position.set(-0.55, 1.25, 0);
    arm2.position.set(0.55, 1.25, 0);
  ...
  getHorizontalCollisionAt(x, z) {
    const feetY = Math.floor(this.position.y + 0.1);
    const headY = Math.floor(this.position.y + 1.7);
    const xSamples = [Math.floor(x - PLAYER_RADIUS + 0.5), Math.floor(x + PLAYER_RADIUS + 0.5)];
    const zSamples = [Math.floor(z - PLAYER_RADIUS + 0.5), Math.floor(z + PLAYER_RADIUS + 0.5)];

    for (const bx of xSamples) {
      for (const bz of zSamples) {
        if (this.world.hasBlock(bx, feetY, bz) || this.world.hasBlock(bx, headY, bz)) return true;
      }
    }
    return false;
  }
  ...
  update(dt) {
    ...
    this.forward.set(0, 0, -1).applyAxisAngle(THREE.Object3D.DEFAULT_UP, this.yaw);
    ...
    if (!this.getHorizontalCollisionAt(nextX, this.position.z)) this.position.x = nextX;
    else this.velocity.x = 0;

    const nextZ = this.position.z + this.velocity.z * dt;
    if (!this.getHorizontalCollisionAt(this.position.x, nextZ)) this.position.z = nextZ;
    ...
    if (this.thirdPerson) {
      const offset = new THREE.Vector3(0, 2.2, 4).applyAxisAngle(THREE.Object3D.DEFAULT_UP, this.yaw);
      this.camera.position.set(this.position.x + offset.x, this.position.y + offset.y, this.position.z + offset.z);
      this.camera.lookAt(this.position.x, this.position.y + 1.3, this.position.z);
